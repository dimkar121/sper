...
# --- Configuration ---
BUDGET_STEPS = [ 2_000,  3_000, 4_000,  5_000, 6_800]
CSV_FILENAME = './results/amazon.csv'
...
def load_and_embed_data(abt_path='./data/Amazon.csv', buy_path='./data/GoogleProducts.csv', gt_path='./data/truth_amazon_google.csv'):
    ...
    gt_pairs = set(zip(df_gt['idAmazon'], df_gt['idGoogle']))
     
        

    # Text Preprocessing
    df_abt['text'] = (df_abt['name'].fillna('') + " " + df_abt['description'].fillna('')  + df_abt['manufacturer'].fillna('') ).astype(str)
    df_buy['text'] = (df_buy['name'].fillna('') + " " + df_buy['description'].fillna('') + df_buy['manufacturer'].fillna('') ).astype(str)
    ...
    return  vectors_buy, df_buy['id'].values,   vectors_abt, df_abt['id'].values,  gt_pairs
