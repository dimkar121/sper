...
BUDGET_STEPS = [100_000, 500_000, 1_000_000, 3_000_000, 5_000_000]
CSV_FILENAME = './results/voters.csv'
...
def load_and_embed_data(abt_path='./data/test_voters_A_1M.txt', buy_path='./data/test_voters_B_1M.txt', gt_path='./data/truth_VOTERS.csv'):
    print("1. Loading & Preprocessing Data...")
    try:
        COLS_TO_USE = ["id","surname","name","address","town","ps"]
        df_abt = pd.read_csv(abt_path, encoding='unicode_escape', names=COLS_TO_USE, on_bad_lines='skip' )
        df_buy = pd.read_csv(buy_path, encoding='unicode_escape', names=COLS_TO_USE, on_bad_lines='skip' ) 
        df_gt = pd.read_csv(gt_path, encoding="unicode_escape")
    ...
    # Text Preprocessing
    df_abt['text'] = (df_abt['surname'].fillna('') + " "+ df_abt['name'].fillna('')+" "+ df_abt['address'].fillna('')   ).astype(str)
    df_buy['text'] = (df_buy['surname'].fillna('') + " "+ df_buy['name'].fillna('')+ " " + df_buy['address'].fillna('')   ).astype(str)
