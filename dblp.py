...
# --- Configuration ---
#BUDGET_STEPS = [500_000, 2_000_000, 3_000_000, 4_000_000, 5_000_000]
BUDGET_STEPS = [1_000_000, 2_000_000, 5_000_000, 10_000_000, 15_000_000]
CSV_FILENAME = './results/dblp.csv'
...
def load_and_embed_data(abt_path='./data/test_dblp_A.txt', buy_path='./data/test_dblp_B.txt', gt_path='./data/truth_DBLP.csv'):
    print("1. Loading & Preprocessing Data...")
    try:
        COLS_TO_USE = ["id","author1","author2","title","year"]
        df_abt = pd.read_csv(abt_path, encoding='utf-8', names=COLS_TO_USE, on_bad_lines='skip' ) #, nrows=1_000_000)
        df_buy = pd.read_csv(buy_path, encoding='utf-8', names=COLS_TO_USE, on_bad_lines='skip' ) #, nrows=1_000_000)
        df_gt = pd.read_csv(gt_path, encoding="utf-8") #,nrows=500_000)
    except FileNotFoundError:
        print("Error: CSV files not found.")
        return None, None, None, None, None

    gt_pairs = set(zip(df_gt['id1'], df_gt['id2']))
    
    # Text Preprocessing
    df_abt['text'] = (df_abt['author1'].fillna('') + " "+ df_abt['author2'].fillna('')+" "  + df_abt['title'].fillna('')+ " " +df_abt['year'].fillna('')  ).astype(str)
    df_buy['text'] = (df_buy['author1'].fillna('') + " "+ df_buy['author2'].fillna('')+ " " + df_buy['title'].fillna('')+ " " +df_buy['year'].fillna('') ).astype(str)
    ...
    print(f"Stream size={len(vectors_buy)} {len(vectors_buy)*5} ")
...
def build_faiss_index(vectors, d):
    ...
    # Training is not needed for HNSWFlat, but we can set efConstruction
    index.hnsw.efConstruction = 40  # Trade-off between build time and accuracy
    index.add(vectors)
    return index
...
def run_single_sper_pass(
    ...
    for i in range(0, stream_size, window_size):
        ...
        # 1. Retrieval (FAISS Batch Search)
        # distances corresponds to cosine similarity (since normalized)
        ...
        for local_idx in range(len(window_ids)):
            ...
            for rank in range(top_k):
                ...
                if idx_neighbor < 0: continue # Should not happen with valid top_k
                
                w_score = sims[local_idx][rank]
                target_id = ids_abt[idx_neighbor]
                
                # SPER Probability Formula
