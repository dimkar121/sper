...
CSV_FILENAME = './results/scholar.csv'
...
def load_and_embed_data(abt_path='./data/Scholar.csv', buy_path='./data/DBLP2.csv', gt_path='./data/truth_Scholar_DBLP.csv'):
    print("1. Loading & Preprocessing Data...")
    try:
        df_abt = pd.read_csv(abt_path, encoding='utf-8')
        df_buy = pd.read_csv(buy_path, encoding='utf-8')
        df_gt = pd.read_csv(gt_path)
    except FileNotFoundError:
        print("Error: CSV files not found.")
        return None, None, None, None, None

    gt_pairs = set(zip(df_gt['idScholar'], df_gt['idDBLP']))
