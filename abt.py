...
# --- Configuration ---
BUDGET_STEPS = [1000, 2000, 3_000, 4_000, 5_500]
CSV_FILENAME = './results/abt.csv'
...
def load_and_embed_data(abt_path='./data/Abt.csv', buy_path='./data/Buy.csv', gt_path='./data/truth_abt_buy.csv'):
    ...
    gt_pairs = set(zip(df_gt['idAbt'], df_gt['idBuy']))
    
    # Text Preprocessing
    df_abt['text'] = (df_abt['name'].fillna('') + " " + df_abt['description'].fillna('')).astype(str)
    df_buy['text'] = (df_buy['name'].fillna('') + " " + df_buy['description'].fillna('')).astype(str)
