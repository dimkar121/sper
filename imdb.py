...
# --- Configuration ---
BUDGET_STEPS = [30_000, 50_000,80_000, 100_000, 116_000 ]
CSV_FILENAME = './results/imdb.csv'

# Dataset Paths & Columns
PATH_RAW_A = './data/imdb.csv'
PATH_RAW_B = './data/dbpedia.csv'
PATH_GT = './data/truth_imdb_dbpedia.csv' # Assumed TSV based on typical data format
ID_COL_A = 'D1'
ID_COL_B = 'D2'
...
def load_and_embed_data(
    ...
    try:
        ...
        try:
            df_a = pd.read_csv(path_a,sep="|",  encoding="utf-8")
            df_b = pd.read_csv(path_b, sep="|", encoding="utf-8")
        ...
        print(f"df_a {len(df_a)}, df_b {len(df_b)} truth {len(df_gt)} ")
    ...
    # Text Preprocessing
    df_a = df_a.dropna(subset=['title'])
    df_b = df_b.dropna(subset=['title'])
    ...
    for index, row in df_gt.iterrows():
        ...
        if not idimdb in df_a['id'].values or not iddbpedia in df_b['id'].values:
              print(f"found {idimdb} {iddbpedia}")
              continue
            
        gt_pairs.add((idimdb, iddbpedia)) # Store as (Amazon, Walmart) tuple
